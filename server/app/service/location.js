...
class LocationService extends Service {
    async getLocationList(options) {
        const ctx = this.ctx
        let res = await db.query('select * from location where user_id = ?', ctx.session.userId)
        if(res){
    ...
    async addLocation(options){
        const ctx = this.ctx
        let res = await db.query('insert into location (user_id, coordinate, remarks, name, create_time) values (?, ?, ?, ?, ?)', [ctx.session.userId, options.coordinate, options.remarks, options.name, options.createTime])
        if(res){
    ...
    async deleteLocation(options){
        let res = await db.query('delete from location where location_id = ?', [options.locationId])
        if(res){
