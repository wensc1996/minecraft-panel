...
class DisposeService extends Service {
    ...
    async updateGameDispose(options) {
        let res = await db.query('update dispose set game_port = ?, panel_port = ?, max_players = ?, max_memory_size = ?, min_memory_size = ?, jar_name = ?, java_path = ?, work_path = ?', [options.gamePort, options.panelPort, options.playerNum, options.maxMemorySize, options.minMemorySize, options.jarName, options.javaPath, options.workPath])
        if(res){
