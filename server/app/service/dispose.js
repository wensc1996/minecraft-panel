...
class DisposeService extends Service {
    ...
    async updateGameDispose(options) {
        let mysql = new Mysql()
        let res = await mysql.action('update dispose set game_port = ?, panel_port = ?, max_players = ?, max_memory_size = ?, min_memory_size = ?, jar_name = ?', [options.gamePort, options.panelPort, options.playerNum, options.maxMemorySize, options.minMemorySize, options.jarName])
        if(res){
