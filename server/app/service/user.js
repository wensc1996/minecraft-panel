...
class UserService extends Service {
    ...
    async getUserList() {
        ...
        let res = await db.query('select user_id,player_id,login_ip,role_name as role from user,role where user.role_id = role.role_id')
        return new Response({code: 1, msg: '查询成功', data : res})
    }
    async updatePassword(options) {

        let oldInfo = await db.query('select * from user where user_id = ?', options.userId)
        if(oldInfo[0].password == this.md5(options.oldPassword)){
            let res = await db.query('update user set password = ? where user_id = ?', [this.md5(options.password), options.userId])
            if(res){
    ...
    async addNewUser(options) {
        ...
        let res = await db.query('insert into user(player_id, login_ip, role_id, password) values (?, ?, ?, ?)', [options.playerId, options.loginIp, options.roleId, this.md5(options.password)])
        if(res){
    ...
    async updatePlayerId(options) {
        ...
        let res = await db.query('update user set player_id = ? where user_id = ?', [options.playerId, options.userId])
        if(res){
    ...
    async deleteUser(options) {
        ...
        let res = await db.query('delete from user where user_id = ?', options.userId)
        if(res){
