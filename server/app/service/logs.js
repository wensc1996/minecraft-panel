...
class LogsService extends Service {
    async getLogList(options) {
        const ctx = this.ctx
        // 租户隔离：仅本租户日志
        let res = await db.query('select log_id, COALESCE(u.account, \'-\') as account_id, operation,date_format(op_time, "%Y-%m-%d %H:%i:%s") as op_time from logs l left join user u on l.user_id = u.user_id where l.tenant_id = ? order by l.op_time desc limit ?,?', [ctx.tenantId, (options.current - 1) * options.pageSize, options.pageSize])
